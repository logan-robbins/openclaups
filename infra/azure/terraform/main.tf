module "shared_infra" {
  ...
  resource_group_name         = local.environment.resource_group_name
  virtual_network_name        = local.environment.virtual_network_name
  subnet_name                 = local.environment.subnet_name
  network_security_group_name = local.environment.network_security_group_name
  address_space               = local.environment.address_space
  subnet_prefixes             = local.environment.subnet_prefixes
...
module "image_gallery" {
  ...
  gallery_name           = local.environment.gallery_name
  image_definition_name  = local.environment.image_definition_name
  image_identifier       = local.image_identifier
  hyper_v_generation     = try(local.environment.hyper_v_generation, "V2")
  trusted_launch_enabled = try(local.environment.trusted_launch_enabled, true)
  tags                   = local.shared_tags
}
